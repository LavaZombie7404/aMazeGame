...
- **REDMAGIC 10 Pro (and likely other gaming phones) returns empty `adb logcat`.** `adb logcat -d` prints zero lines even after `adb logcat -c` + a fresh app launch + `adb shell log -p i -t TestTag 'hello'` — so the diagnostic shortcut isn't available. Plan B when the app crashes silently: install [pidcat](https://github.com/JakeWharton/pidcat) on top of a working ROM, or unlock "Disable log access restriction" in Developer options if present, or fall back to `adb shell am start` with a known intent + checking `ps -A | grep <pkg>` to see if the process even survives launch.
